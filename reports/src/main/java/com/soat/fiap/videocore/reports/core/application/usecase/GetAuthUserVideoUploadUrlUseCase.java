...
/**
 * Caso de uso responsável por obter uma URL de upload de um vídeo para
 * processamento. Para o usuário autenticado.
 */
...
public class GetAuthUserVideoUploadUrlUseCase {
	 ...
	 *
	 * <p>
	 * A URL retornada permite apenas operações de criação e escrita, o caminho do
	 * arquivo é construído a partir do {@code userId} e {@code requestId}. Utiliza
	 * um tempo de expiração de 30 minutos.
	 *
	 * @param videoNames
	 *            Nome do vídeo (arquivo com extensão)
	...
	public List<VideoUploadUrlOutput> getVideoUploadUrl(List<String> videoNames) {
		...
		if (videoNames.stream().anyMatch(name -> name == null || name.isBlank()))
			throw new VideoException(
					"A lista de nome dos vídeos não pode conter nomes em branco para criação da URL de upload.");
