...
/**
 * Mapper MapStruct para converter metadados do Blob Storage em
 * {@link VideoDto}.
 */
...
public interface VideoBlobMapper {
	 ...
	 *
	 * @param metadata
	 *            mapa de metadados do blob (ex.: user_id, request_id)
	 * @param blobName
	 *            nome/caminho do blob
	 * @return DTO com os dados extraídos
	 */
	@Mapping(target = "videoName", expression = "java(extractVideoName(blobName))")
	@Mapping(target = "userId", expression = "java(metadata.get(\"user_id\"))")
	@Mapping(target = "requestId", expression = "java(metadata.get(\"request_id\"))")
	@Mapping(target = "minuteFrameCut", expression = "java(Long.parseLong(metadata.get(\"frame_cut\")))")
