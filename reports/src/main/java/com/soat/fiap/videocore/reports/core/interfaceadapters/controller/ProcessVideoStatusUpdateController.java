...
/**
 * Controller responsável por orquestrar reportes de atualizações no status de
...
public class ProcessVideoStatusUpdateController {
	...
	/**
	 * Processa a atualização no status de processamento de um vídeo através do
	 * payload de evento: {@link ProcessVideoStatusUpdatePayload}.
	 *
	 * @param payload
	 *            Payload de atualização de status do vídeo.
	 */
	...
	public void processVideoStatusUpdate(ProcessVideoStatusUpdatePayload payload) {
		...
		var existingReport = getExistingReportUseCase.getExistingReport(input.userId(), input.requestId(),
				input.videoName(), input.percentStatusProcess());
