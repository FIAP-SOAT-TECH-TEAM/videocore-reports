...
class ProcessVideoStatusUpdateControllerTest {
	...
	void shouldCreateAndSaveReportWhenNoExistingReport() {
		...
		when(getExisting.getExistingReport(any(), any(), any(), any())).thenReturn(Optional.empty());
		when(save.saveReport(report)).thenReturn(report);
		when(report.getStatus()).thenReturn(ProcessStatus.STARTED);

		var controller = new ProcessVideoStatusUpdateController(mapper, create, update, getExisting, save, notify,
				publishError);
	...
	void shouldUpdateAndPublishErrorWhenReportFails() {
		...
		when(getExisting.getExistingReport(any(), any(), any(), any())).thenReturn(Optional.of(existingReport));
